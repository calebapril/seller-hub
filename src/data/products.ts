...
//NOTE - Products Categories for landing page.
...
export const CATEGORIES = {
  ...
  Cakes: ["Sweet Intentions", "Secret Crush", "Pure Bliss Cake", "Peach Cream"],
};

export const CAKE_BRANDS = [
  "Sweet Treats Bakery",
  "Heavenly Cakes",
  "Sugar Rush",
  "Divine Bites",
...
const generateProducts = (count: number): Product[] => {
  return Array.from({ length: count }).map((_, i) => {
    ...
    if (categoryKey === "Cakes") {
      brand = getRandomItem(CAKE_BRANDS);

      name = `${getRandomItem(CAKE_ADJECTIVES)} ${subcategory} Cake for ${getRandomItem(CAKE_OCCASIONS)}`;
    } else {
      brand = getRandomItem(BRANDS);

      name = `${brand} ${getRandomItem(ADJECTIVES)} ${subcategory} ${getRandomItem(NOUNS)}`;
    }
    ...
    if (categoryKey === "Cakes") {
      description = `
    <p>Make your ${getRandomItem(CAKE_OCCASIONS)} extra special with this delicious ${subcategory} cake.</p>
    <p>Freshly baked with love, perfect for sharing unforgettable moments.</p>
  `;
    } else {
      description = `
    <p>Experience the ultimate in quality with the ${name}.</p>
  `;
    }
    ...
    return {
      ...
      specifications:
        categoryKey === "Cakes"
          ? {
              Flavor: getRandomItem(["Chocolate", "Vanilla", "Strawberry"]),
              Size: getRandomItem(["Small", "Medium", "Large"]),
              Weight: `${getRandomInt(1, 5)}kg`,
              Type: "Freshly Baked",
            }
          : {
              Brand: brand,
